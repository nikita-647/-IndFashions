...
const TermsAndConditions = () => {
  ...
  const loadSavedVersions = async () => {
    try {
      const keys = await window.storage.list("terms_conditions:");
      if (keys && keys.keys) {
  ...
  const loadVersion = async (id) => {
    try {
      const result = await window.storage.get(`terms_conditions:${id}`);
      if (result) {
  ...
  const deleteVersion = async (id) => {
    if (window.confirm("Are you sure you want to delete this version?")) {
      try {
        await window.storage.delete(`terms_conditions:${id}`);
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Ind Fashions - Legal Content Management
                </p>
              </div>
            </div>
